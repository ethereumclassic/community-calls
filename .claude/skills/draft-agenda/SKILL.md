...
1. Preserve the existing frontmatter unchanged, except `agendaSummary` (below)
2. Replace placeholder content (e.g. "Agenda TBD") with the drafted agenda
3. Set `agendaSummary` in the frontmatter: a one-line teaser listing the agenda topics, roughly 80 characters, no trailing period. It goes into the share snippet and the X post, and the build fails if the post exceeds 280 characters. Update it whenever the agenda topics change
4. Inform the user the draft has been saved and offer to iterate

## Important Notes

- Do NOT modify the frontmatter fields other than `agendaSummary`
