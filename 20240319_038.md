...
A token that can be purchased / auctioned no more than 10% of each block's gas fee. Fees go to miners.

That 10% gas is "used" but does not contain much data, so doesn't contribute to bloat. 

This reduces the chain growth for that block.
...
The result is that net chain growth is the same (or less), while still allowing developers to make complex transactions / deployments. Combine this with fixed block limit, makes Ronin and Donald both satisfied.

This, of course, would come at the cost complexity though, but implemented mostly in SC layer.
