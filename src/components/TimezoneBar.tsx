...
export default function TimezoneBar({ time }: Props) {
  ...
  const timezones = useMemo(() => {
    ...
    const filtered = baseTimezones.filter((tz) => {
      ...
      return tzTime.time !== userTime.time || tzTime.dayOffset !== userTime.dayOffset;
    });
    ...
    for (const tz of filtered) {
      ...
      if (tz.isCenter && !inserted && userOffset >= 0) {
        ...
        const nextNonCenter = filtered.find(
          (t) => !t.isCenter && t.offset >= 0 && filtered.indexOf(t) > filtered.indexOf(tz)
        );
  ...
  const renderTimezoneItem = (tz: TimezoneEntry) => {
    const { time: displayTime, dayOffset } = calculateLocalTime(utcTime, tz.offset);
    ...
    if (tz.isUserTime) {
      return (
        ...
        >
          ...
          {dayOffset !== 0 && (
            <span className="text-[9px] text-etc-green/80">{dayOffset > 0 ? '+1' : '-1'}</span>
          )}
        </div>
      );
    }
  ...
  const renderMobileItem = (tz: TimezoneEntry) => {
      ...
      return (
        ...
        >
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />
            <span className="text-sm font-mono text-orange-400 font-medium">{tz.city}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="text-sm font-mono text-orange-300 font-semibold">{displayTime}</span>
            {dayOffset !== 0 && (
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono text-gray-500 w-10">{tz.abbr}</span>
          <span className="text-sm font-mono text-gray-400">{tz.city}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-sm font-mono text-gray-200 font-semibold">{displayTime}</span>
          {dayOffset !== 0 && (
            <span className="text-[10px] font-mono text-etc-green/80">{dayOffset > 0 ? '+1' : '-1'}</span>
          )}
        </div>
      </div>
    );
  ...
  const mobileItems = timezones.filter((tz) => !tz.isCenter).sort((a, b) => a.offset - b.offset);
        ...
        <div className="flex items-center justify-center gap-1 text-xs font-mono min-w-max">
          ...
          {centerZone && (
            <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-etc-green/10 border border-etc-green/30 flex-shrink-0">
              ...
              <span className="text-etc-green font-semibold">{centerZone.abbr}</span>
      ...
      <div className="md:hidden">
        <details className="timezone-dropdown">
          <summary className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-etc-green/10 border border-etc-green/30 cursor-pointer list-none select-none">
            <span className="w-1.5 h-1.5 rounded-full bg-etc-green animate-pulse-glow" />
            <span className="text-etc-green font-mono text-sm font-semibold">UTC</span>
